...
- A separate **pruning method experiment** mirrors the QAT vs PTQ one: `model/experiments/prune_post_vs_train.py` compares dense fp32 against **coremltools** magnitude pruning introduced two ways — `prune-train` (training-time gradual ramp via `ct.optimize.torch.pruning.MagnitudePruner`) and `prune-post` (one-shot cut of the trained baseline, the deploy-time equivalent of `ct.optimize.coreml.prune_weights`) — swept over several target sparsities on the same 200k/10k search slice, seed and hparams. `model/pruning_coreml.py` holds `CoreMLMagnitudePruner`, an adapter that exposes the Trainer's pruner interface over Apple's optimizer (no QAT in this experiment; PTQ is applied post-training). Run: `python -m model.experiments.prune_post_vs_train --arch <arch> --baseline --prune-train --prune-post --sparsities 0.3 0.5 0.75`.
